...
public class CSVReader {
    ...
    public static List<Employee> readEmployeesFromCSV(String filepath) throws Exception {
        ...
        try (InputStream is = CSVReader.class.getClassLoader().getResourceAsStream(filepath);
            ...
            // Temporary map from EmployeeNumber to Employee for quick lookup
            Map<Integer, Employee> employeeMap = new HashMap<>();

            // Temporary map from Employee to supervisor full name (to resolve later)
            ...
            while ((line = csvReader.readNext()) != null) {
                ...
                // Create Position with null supervisor for now
                Position position = new Position(positionName, null);

                // No password column, default to empty string
                String password = "";
            ...
            // After all employees loaded, resolve supervisors by full name
            for (Employee emp : employees) {
                String supName = supervisorNames.get(emp);
                if (!supName.isEmpty()) {
                    // Find supervisor by full name, case insensitive
                    Employee supervisor = findEmployeeByFullName(employees, supName);
